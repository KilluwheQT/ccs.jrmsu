<template>
  <div class="space-y-12">
    ...
    <section class="text-center py-16 bg-gradient-to-br from-purple-900/50 via-purple-800/30 to-gray-900/50 rounded-2xl backdrop-blur-sm border border-purple-500/20">
      <div class="max-w-4xl mx-auto px-4">
        <h1 class="text-4xl md:text-6xl font-bold text-purple-300 mb-6">
          CCS <span class="text-purple-400">Events</span>
        </h1>
    ...
    <!-- Upcoming Events -->
    <section class="card">
      <h2 class="text-3xl font-bold text-purple-300 mb-8 text-center">Upcoming Events</h2>
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div 
          v-for="event in filteredUpcomingEvents" 
          :key="event.id"
          class="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-purple-500/20 overflow-hidden shadow-sm hover:shadow-md transition-shadow"
        >
          <div class="p-6">
            ...
            <h3 class="text-lg font-semibold text-gray-200 mb-2">{{ event.title }}</h3>
            <p class="text-gray-300 text-sm mb-4">{{ event.description }}</p>
    ...
    <!-- Past Events -->
    <section class="card">
      <h2 class="text-3xl font-bold text-purple-300 mb-8 text-center">Past Events</h2>
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div 
          v-for="event in filteredPastEvents" 
          :key="event.id"
          class="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-purple-500/20 overflow-hidden"
        >
          <div class="p-6">
            <div class="flex items-center justify-between mb-4">
              <span :class="getEventBadgeClass(event.category)" class="px-3 py-1 rounded-full text-xs font-medium">
    ...
    <!-- Event Calendar -->
    <section class="card">
      <h2 class="text-3xl font-bold text-purple-300 mb-8 text-center">Event Calendar</h2>
      <div class="bg-gray-800/50 backdrop-blur-sm rounded-lg p-8 text-center border border-purple-500/20">
        <div class="max-w-md mx-auto">
          <svg class="w-24 h-24 text-gray-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          <h3 class="text-lg font-semibold text-gray-200 mb-2">Interactive Calendar</h3>
    ...
    <!-- Call to Action -->
    <section class="text-center py-12 bg-gradient-to-br from-purple-900/50 via-purple-800/30 to-gray-900/50 rounded-2xl backdrop-blur-sm border border-purple-500/20">
...
const getEventBadgeClass = (category) => {
  const classes = {
    'Workshop': 'bg-blue-900/30 text-blue-300 border border-blue-600/30',
    'Seminar': 'bg-green-900/30 text-green-300 border border-green-600/30',
    'Conference': 'bg-purple-900/30 text-purple-300 border border-purple-600/30',
    'Competition': 'bg-red-900/30 text-red-300 border border-red-600/30',
    'Social': 'bg-yellow-900/30 text-yellow-300 border border-yellow-600/30'
  }
  return classes[category] || 'bg-gray-800/30 text-gray-300 border border-gray-600/30'
}
